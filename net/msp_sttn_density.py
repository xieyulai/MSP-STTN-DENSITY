...
class Conv_block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, dilation, scale_factor=1, residual=True, bn=True,
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels, encoding_dim):
        ...
        self.conv1 = Conv_block(input_channels, encoding_dim // 4, kernel_size=3, stride=2, dilation=1,
                                residual=False)
        self.conv2 = Conv_block(encoding_dim // 4, encoding_dim // 4, kernel_size=3, stride=2, dilation=1,
                                residual=True)
        self.conv3 = Conv_block(encoding_dim // 4, encoding_dim // 2, kernel_size=3, stride=2, dilation=1,
                                residual=True)
        self.conv4 = Conv_block(encoding_dim // 2, encoding_dim, kernel_size=3, stride=1, dilation=2,
                                residual=True)
...
class Decoder(nn.Module):
    ...
    def __init__(self, output_channels, encoding_dim, Using_skip, Activation):
        ...
        self.conv5 = Conv_block(encoding_dim, encoding_dim // 4, kernel_size=3, stride=1, dilation=1,scale_factor=1)
        self.conv6 = Conv_block(encoding_dim // 2 if Using_skip else encoding_dim // 4,
                                encoding_dim // 8, kernel_size=3, stride=1, dilation=1,scale_factor=2)
        ...
        self.conv8 = Conv_block(encoding_dim // 4 if Using_skip else encoding_dim // 8,
                                output_channels, kernel_size=3, stride=1, dilation=1,scale_factor=2,
...
class Multi_patch_transfomer(nn.Module):
    ...
    def multi_patch_forward(self, c, q, mask):
        ...
        attention_score = []
        for i in range(self.scale_num):
            z,score = self.multi_patch_transformer[i](c, q, mask)
            output.append(z)
...
class Prediction_Model(nn.Module):
    ...
    def __init__(self, mcof, Length, Width, Height, Input_dim, Patch_list, Encoding_dim, Embedding_dim,
        ...
        encoding_h = Height
        self.length = Length
        self.encoding_h = encoding_h
        self.encoding_w = encoding_w
        ...
        self.linear_tim = nn.Sequential(
            nn.Conv2d(self.embedding_dim, Input_dim, kernel_size=3, padding=1),
            nn.LeakyReLU(0.2, inplace=True))

        self.feedforward_tim = nn.Sequential(
            nn.Linear(Length * Input_dim * Height * Width, 64),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(p=0.2),
            nn.Linear(64, 24),
            nn.LeakyReLU(0.2, inplace=True),
...
if __name__ == '__main__':
    ...
    tensor  = torch.randn(4, 6, 1, 200, 200)
